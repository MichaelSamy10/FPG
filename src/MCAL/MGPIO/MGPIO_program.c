...
void MGPIO_voidSetPinMode(u8 Copy_u8Port, u8 Copy_u8Pin, u8 Copy_u8Mode)
{
	...
	{
	case MGPIO_u8PORTA:
		MGPIOA->MODER &= ~((0b11) << (Copy_u8Pin * 2));		/*select the pin in the MODER register*/
		MGPIOA->MODER |= (Copy_u8Mode << (Copy_u8Pin * 2)); /*select the mode in the MODER register*/
		break;												/*end of case port A*/
	case MGPIO_u8PORTB:
		MGPIOB->MODER &= ~((0b11) << (Copy_u8Pin * 2));		/*select the pin in the MODER register*/
		MGPIOB->MODER |= (Copy_u8Mode << (Copy_u8Pin * 2)); /*select the mode in the MODER register*/
		break;												/*end of case port B*/
	case MGPIO_u8PORTC:
		MGPIOC->MODER &= ~((0b11) << (Copy_u8Pin * 2));		/*select the pin in the MODER register*/
...
void MGPIO_voidSetOutputType(u8 Copy_u8Port, u8 Copy_u8Pin, u8 Copy_u8OutputType)
{
	...
	{
	case MGPIO_u8PORTA:
		MGPIOA->OTYPER &= ~((1) << (Copy_u8Pin));			   /*select the pin in the OTYPER register*/
		MGPIOA->OTYPER |= (Copy_u8OutputType << (Copy_u8Pin)); /*select the output type in the OTYPER register*/
		break;												   /*end of case port A*/
	case MGPIO_u8PORTB:
		MGPIOB->OTYPER &= ~((1) << (Copy_u8Pin));			   /*select the pin in the OTYPER register*/
		MGPIOB->OTYPER |= (Copy_u8OutputType << (Copy_u8Pin)); /*select the output type in the OTYPER register*/
		break;												   /*end of case port B*/
	case MGPIO_u8PORTC:
		MGPIOC->OTYPER &= ~((1) << (Copy_u8Pin));			   /*select the pin in the OTYPER register*/
...
void MGPIO_voidSetOutputSpeed(u8 Copy_u8Port, u8 Copy_u8Pin, u8 Copy_u8Speed)
{
	...
	{
	case MGPIO_u8PORTA:
		MGPIOA->OSPEEDER &= ~((0b11) << (Copy_u8Pin * 2));		/*select the pin in the OSPEEDER register*/
		MGPIOA->OSPEEDER |= (Copy_u8Speed << (Copy_u8Pin * 2)); /*select the output SPEED in the OSPEEDER register*/
		break;													/*end of case port A*/
	case MGPIO_u8PORTB:
		MGPIOB->OSPEEDER &= ~((0b11) << (Copy_u8Pin * 2));		/*select the pin in the OSPEEDER register*/
		MGPIOB->OSPEEDER |= (Copy_u8Speed << (Copy_u8Pin * 2)); /*select the output SPEED in the OSPEEDER register*/
		break;													/*end of case port B*/
	case MGPIO_u8PORTC:
		MGPIOC->OSPEEDER &= ~((0b11) << (Copy_u8Pin * 2));		/*select the pin in the OSPEEDER register*/
...
void MGPIO_voidSetPullType(u8 Copy_u8Port, u8 Copy_u8Pin, u8 Copy_u8PullType)
{
	...
	{
	case MGPIO_u8PORTA:
		MGPIOA->PUPDR &= ~((0b11) << (Copy_u8Pin * 2));			/*select the pin in the PUPDR register*/
		MGPIOA->PUPDR |= (Copy_u8PullType << (Copy_u8Pin * 2)); /*select the INPUT type in the PUPDR register*/
		break;													/*end of case port A*/
	case MGPIO_u8PORTB:
		MGPIOB->PUPDR &= ~((0b11) << (Copy_u8Pin * 2));			/*select the pin in the PUPDR register*/
		MGPIOB->PUPDR |= (Copy_u8PullType << (Copy_u8Pin * 2)); /*select the INPUT type in the PUPDR register*/
		break;													/*end of case port A*/
	case MGPIO_u8PORTC:
		MGPIOC->PUPDR &= ~((0b11) << (Copy_u8Pin * 2));			/*select the pin in the PUPDR register*/
...
u8 MGPIO_u8GetPinValue(u8 Copy_u8Port, u8 Copy_u8Pin)
{
	...
	{
	case MGPIO_u8PORTA:
		Local_PINvalue = GET_BIT(MGPIOA->IDR, Copy_u8Pin);
		break; /*get the value of the pin from IDR(PORTA)register*/
	case MGPIO_u8PORTB:
		Local_PINvalue = GET_BIT(MGPIOB->IDR, Copy_u8Pin);
		break; /*get the value of the pin from IDR(PORTB)register*/
	case MGPIO_u8PORTC:
		Local_PINvalue = GET_BIT(MGPIOC->IDR, Copy_u8Pin);
		break; /*get the value of the pin from IDR(PORTC)register*/
	}
...
void MGPIO_voidSetPinValue(u8 Copy_u8Port, u8 Copy_u8Pin, u8 Copy_u8Value)
{
	...
	if (Copy_u8Value == MGPIO_u8LOW)
	{
		...
		case MGPIO_u8PORTA:
			...
			break; /*SET the value of the pin in ODR (PORTA) register to  0 */
		case MGPIO_u8PORTB:
			CLR_BIT(MGPIOB->ODR, Copy_u8Pin);
			break; /*SET the value of the pin in ODR (PORTB) register to  0 */
		case MGPIO_u8PORTC:
			CLR_BIT(MGPIOC->ODR, Copy_u8Pin);
			break; /*SET the value of the pin in ODR (PORTC) register to  0 */
		}
	}
	else if (Copy_u8Value == MGPIO_u8HIGH)
	{
		...
		case MGPIO_u8PORTA:
			...
			break; /*SET the value of the pin in ODR (PORTA) register to  1 */
		case MGPIO_u8PORTB:
			SET_BIT(MGPIOB->ODR, Copy_u8Pin);
			break; /*SET the value of the pin in ODR (PORTB) register to  1 */
		case MGPIO_u8PORTC:
			SET_BIT(MGPIOC->ODR, Copy_u8Pin);
			break; /*SET the value of the pin in ODR (PORTC) register to  1 */
		}
	}
}
...
void MGPIO_voidSetPortValue(u8 Copy_u8Port, u8 Copy_u8Value)
{
	...
	case MGPIO_u8PORTA:
		...
		break; /*set the required value to all the pins in port A*/
	case MGPIO_u8PORTB:
		MGPIOB->ODR = Copy_u8Value;
		break; /*set the required value to all the pins in port B*/
	case MGPIO_u8PORTC:
		MGPIOC->ODR = Copy_u8Value;
		break; /*set the required value to all the pins in port C*/
	}
}
